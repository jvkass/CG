...
bool Vector4::operator==(Vector4& v)
	...
	{
		Vector4 aux=plano[1]-plano[0];
		Vector4 aux2=plano[2]-plano[0];
		aux.Cross(aux2);
		Vector4 normal=aux.Cross(aux2).Normalize();
		//Vector4 normal = (plano[1] - plano[0]).Cross(plano[2] - plano[0]).Normalize();
		double op1=this->Dot(normal) * 2.0f;
		Vector4 op2=normal*op1;
		//return *this - normal * (2.0f * this->Dot(normal));
		Vector4 op3 = *this - op2;
		return op3;
	}
